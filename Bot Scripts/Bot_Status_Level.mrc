...
on *:start:{
  ...
  set -e %adminst 999
  ; set the minimum  and maximum level of users you will be adding on the next 2 lines
  set -e %levelst 19
  set -e %maxst 60
}
on *:TEXT:!status*:#:{
  ...
  if ($ulevel < %adminst) { msg $chan $nick Who the Friggin' heck do you think you are? Bite me! | .ignore -cu10 $nick | halt }
    ...
    if ($2 == del) { goto del }
    if ($3 !isnum) || ($3 < %levelst) || ($3 > %maxst) { 
      msg $chan Invalid Command. Syntax is !status ADD Level ( $+ %levelst or less than %maxst $+ )  NICK or !status Del Nick
      HALT
    }
...
on *:TEXT:!status*:?:{
  if ($ulevel < %adminst) { msg $nick Who the Friggin' heck do you think you are? Bite me! | .ignore -cu10 $nick | halt }
    ...
    if ($2 == del) { goto del }
    if ($3 !isnum) || ($3 < %levelst) || ($3 > %maxst) {
      msg $nick Invalid Command. Syntax is !status ADD Level ( $+ %levelst or less than %maxst $+ )  NICK or !status Del Nick
      HALT
    }
